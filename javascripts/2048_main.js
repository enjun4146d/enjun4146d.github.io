...
borad.onmouseup = function(e){
  ...
  var loc = getPointOnCanvas(canvas, x, y);
	var ex = loc.x;
	var ey = loc.y;
	console.log("mouse up at point( x:" + ex + ", y:" + ey + ")");
...
borad.addEventListener('touchstart', function(e){
  ...
  var loc = getPointOnCanvas(canvas, x, y);
	sx = loc.x;
	sy = loc.y;
	console.log("tuch down at point( x:" + sx + ", y:" + sy + ")");
	}
},false
);

borad.addEventListener('touchemove', function(e){
	if (e.targetTouches.length == 1) {
		e.preventDefault();
		console.log("a")
	}
...
borad.addEventListener('touchend', function(e){
  ...
  var loc = getPointOnCanvas(canvas, x, y);
	var ex = loc.x;
	var ey = loc.y;
	console.log("touch up at point( x:" + ex + ", y:" + ey + ")");
	if(Math.abs(sx-ex) < Math.abs(sy-ey)){
		if(sy < ey)move("down");
		else if(sy > ey)move("up");
	}
